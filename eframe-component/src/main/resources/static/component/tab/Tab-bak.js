...
define(function(require, exports, module) {
    ...
    // 切换
    Tab.prototype.activeById =  function(id){
      if(this._isSameActive(id)){ // id可能外部传递过来的，不是li点击的
        return
      }
      var _this = this,
          config =  this.config
      // 查找对应tab数据
      var activeChild = _.find(config, function(c){
            return c.id == id
          })
      if(activeChild){
        _this._renderChild(activeChild.id, activeChild.child)
      }
      this._active(id)
      this.emit('change', id)
    }
