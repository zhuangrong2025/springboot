...
;(function (util) {
    ...
    util.log = function () {
        ...
        if (util.isFunction(console[type])) {
...
;(function (util, config, global) {
    ...
    var PATHS_RE = /^([^/:]+)(\/.+)$/
    var VARS_RE = /{([^{]+)}/g
    var ID_DEBUG_RE = /(-debug\.js$)|(-debug$)/i
    var PLUGIN_RE = /sea-modules\/{1,2}plugin/i
    ...
    function parseAlias(id) {
        ...
        if (alias && isTopLevel(id)) {
            ...
            if (alias.hasOwnProperty(first)) {
                parts[0] = alias[first]
                id = parts.join('/')
                if (config.debug && !PLUGIN_RE.test(id) && !ID_DEBUG_RE.test(id)) {
                    id = id + '-debug'
                }
            }
        }
    ...
    function parsePaths(id) {
        ...
        if (paths && (m = id.match(PATHS_RE)) && util.isString(paths[m[1]])) {
            id = paths[m[1]] + m[2]
        }
    ...
    function parseVars(id) {
        ...
        if (vars && id.indexOf("{") > -1) {
            id = id.replace(VARS_RE, function (m, key) {
                return util.isString(vars[key]) ? vars[key] : m
            })
        }
    ...
    function parseMap(uri) {
        ...
        for (var i = 0; i < map.length; i++) {
            ...
            if (util.isArray(rule) && rule.length === 2) {
                ...
                var moduleId = uri.substring(uri.indexOf(config.base) + config.base.length)
                ret = rule[moduleId] ? config.base + rule[moduleId] : ret
            }
        }
...
;(function (util, config) {
    ...
    util.fetch = function (url, callback, charset) {
        ...
        if (config.noCacheTS) {
            var noCacheTS = config.noCacheTS
            url += (url.indexOf('?') === -1 ? '?' : '&') + noCacheTS
        }
        if(seajs.pluginSDK && seajs.pluginSDK.config && seajs.pluginSDK.config.project_version){
            url += '?version=' + seajs.pluginSDK.config.project_version;
        }
        if (isCSS) {
            node.rel = 'stylesheet'
            node.href = url
    ...
    var styleList = [], styleCssList = {}

    util.importStyle = function (cssText, id, module) {
        if (!cssText || !id || !module) {
            throw 'css内容为空，或引用的路径不存在!'
        }
        var indexOf = util.indexOf(styleList, id);
        if(id.substring(0, 18) === '#base-style-manage'){
            if(indexOf  < 0){
                styleList.push(id);
            }
        } else if (indexOf >= 0) {
        ...
        //ysj  兼容集成发布后，页面多文件夹嵌套时，引用公共组件的图片
        var basePath = location.pathname;
        var releaseIndex = basePath.indexOf('/release');
        if (releaseIndex !== -1) {
            var _rePath = basePath.substring(releaseIndex);
            var _routeCount = _rePath.split('/').length - 2;   //   /的个数
            if (_routeCount > 1) {   //html多层嵌套的情况
                cssText = cssText.replace(/url\(['"]?(.+?)['"]?\)/igm, function (match, submatch) {
                    if (submatch.substring(0, 10) === '../assets/') {
                        var pre = '';
                        for (var i = 0; i < _routeCount; i++) {
                            pre += '../'
                        }
                        var str = pre + 'assets/';
                        var afterStr = submatch.substring(10);
                        var fullUrl = 'url(' + str + afterStr + ')';
                        return fullUrl;
                    }
                    return match;
                });
        ...
        //
        if (module && module.uri.indexOf(doc.location.protocol + "//" + doc.location.host) !== 0) {
            var module_base = util.dirname(module.uri);
            cssText = cssText.replace(/url\(['"]?(.+?)['"]?\)/igm, function (match, submatch) {
                var url = match;
                if (!util.isAbsolute(submatch) //){
                    && !(submatch.toLowerCase().indexOf('#default#vml') >= 0)) {
        ...
        if (!element) {
            ...
            element.id = id;
            element.className = 'theme_class_style';
            if(match){   //ysj
                ...
                head.insertBefore(element, headerAfter);
            } else {
                head.insertBefore(element, head.childNodes[0]);
            }
        }
...
;(function (seajs, util, config) {
    ...
    Module.prototype._use = function (ids, callback) {
        ...
        var baseUri = this.realUri || this.uri;
        if(ids[0].indexOf('/assets/') !== -1){
            baseUri = '';
        }
        //静态资源包加载，不基于sea-modules文件夹做查找，ysj
        //基于当前html的相对路径
        var uris = resolve(ids, baseUri);
    ...
    Module.prototype._compile = function () {
        ...
        require.async = function (ids, callback) {
            //ysj  支持静态js文件异步引入（例如：sui）
            var basePath = location.href;
            var releaseIndex = basePath.indexOf('/release/');
            if (ids.substring(0, 10) === '../assets/' && releaseIndex !== -1) {
                var assPath = ids.substring(2);
                var prePath = basePath.substring(0, releaseIndex);
                var fullPath = prePath + assPath;
                ids = fullPath;
            }
        ...
        if (util.isFunction(factory)) {
            ...
            if(seajs.pluginSDK && seajs.pluginSDK.config && seajs.pluginSDK.config.debug === true){
                var origin = location.origin;
                var oLen = origin.length;
                var tplPath = module.uri.substring(oLen);
                module.exports = '<!- -' + tplPath + ' 开始 -->' + factory + '<!-- ' + tplPath + ' 结束 -->'
            } else {
    ...
    Module._define = function (id, deps, factory, options) {
        ...
        // define(id || deps, factory) || define(factory, options)
        ...
        //define(id || deps, factory, options)
        else if (argsLength === 3 && util.isFunction(deps)) {
            options = factory;
            factory = deps;
            if (util.isArray(id)) {
                deps = id;
        ...
        var meta = {id: id, dependencies: deps, factory: factory, options: options}
        var derivedUri
        ...
        if (document.attachEvent) {
            ...
            if (script) {
                ...
                var index = derivedUri.indexOf(config.noCacheTS)
                if (index > 0) {
                    derivedUri = derivedUri.substring(0, index - 1)
                }
                derivedUri = util.unParseMap(derivedUri)
            }
    ...
    function runInModuleContext(fn, module) {
        var ret = fn(module.require, module.exports, module, module.options)
        if (ret !== undefined) {
            module.exports = ret
        }
    }
    ...
    seajs.use = function (ids, callback) {
        ...
        if(ids && ids[0] !== 'base-common-module'){
            var baseArr = ['base-common-module'];
            globalModule._use(baseArr);
        }
        preload(function () {
...
;(function (seajs, util, config) {
    ...
    match = loaderSrc.match(/seajs-ts=[\w|\d]+/)
    config.noCacheTS = match ? match[0] : ''
    /**
...
/**
 * Check the modules failed to load
 * */
;(function (seajs, util) {
    seajs.check = function (checkId) {
        var failed = [], compiled = [];
        for (var id in seajs.cache) {
            var module = seajs.cache[id];
            (/.js$/).test(id) && module.status <= 2 ? failed.push(id) : compiled.push(module)
        }
        var result = {};

        if (util.isArray(checkId)) {
            failed = checkId
        }
        else if (util.isString(checkId)) {
            failed = []
            failed.push(checkId)
        }

        util.forEach(failed, function (id) {
            var modules = []
            result[id] = modules
            util.forEach(compiled, function (module) {
                module.dependencies && module.dependencies.indexOf(id) >= 0 && modules.push(module.id)
            })
        })
        return result;
    }
